/* Only used GLEW / GLUT for speed - really sucky example, DON'T DO THIS !!!
 * That being said, it shows the basics of using Bullet with your own graphics
 * routines. */
...
void post(int i) { glutPostRedisplay(); }
...
int main(int argc, char** argv) {
	...
	void* groundBody = bodyCreate(uni, groundShape, 0, 0, -5,
								  0);  // 0 mass == static pos 0,0,-5
	...
	for (int i = 2; i < 150; i++) {
		...
		bodySetRotationEuler(fallingBodies[i], .7, 0, 0);
	...
	win = glutCreateWindow("A very nice and simple example");
