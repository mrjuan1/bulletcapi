...
void* universeCreate() {
	...
	u->dynamicsWorld = new btDiscreteDynamicsWorld(
		u->dispatcher, u->broadphase, u->solver, u->collisionConfiguration);

	/* Possibly could have an iteration callback function so all shapes in the
	 * world get passed one at a time to a callback function(s) only used for
	 * clean up for now... */
...
void* bodyGetShape(void* b) { return BODY(b)->getCollisionShape(); }
...
void universeDestroy(void* u) {
	for (int i = UNI(u)->dynamicsWorld->getNumCollisionObjects() - 1; i >= 0;
		 i--) {
		btCollisionObject* obj =
			UNI(u)->dynamicsWorld->getCollisionObjectArray().at(i);
	...
	for (int j = 0; j < UNI(u)->collisionShapes->size(); j++) {
		btCollisionShape* shape =
			(btCollisionShape*)(UNI(u)->collisionShapes->at(j));
		// collisionShapes[j] = 0;
...
void* shapeCreateBox(void* u, float ex, float ey, float ez) {
	btCollisionShape* shape =
		new btBoxShape(btVector3(btScalar(ex), btScalar(ey), btScalar(ez)));
...
void* hinge2Create(void* u, void* bodyA, void* bodyB, Vec anchor,
	...
	btHinge2Constraint* hinge2 =
		new btHinge2Constraint(*BODY(bodyA), *BODY(bodyB), anc, pax, cax);
	UNI(u)->dynamicsWorld->addConstraint(hinge2, collide);

	/* TODO add to global universe list of constraints for iteration / auto
	 * freeing on destroy universe */
...
float hinge2getAngle1(void* h) { return ((btHinge2Constraint*)h)->getAngle1(); }

float hinge2getAngle2(void* h) { return ((btHinge2Constraint*)h)->getAngle2(); }
...
void* hingeCreate(void* uni, void* bodyA, void* bodyB, Vec pivA, Vec rotA,
	...
	btHingeConstraint* hinge =
		new btHingeConstraint(*BODY(bodyA), *BODY(bodyB), localA, localB, refA);
	UNI(uni)->dynamicsWorld->addConstraint(hinge, collide);

	/* TODO research proper way to dispose of constraints later possibly add to
	 * global constraint list for destroyUniverse */
	return (void*)hinge;
}
...
// TODO can't make any sense of this...
// float hingeGetAngle(void* hinge) {
// return ((btHingeConstraint*)hinge)->getHingeAngle();
// return
// ((btHingeAccumulatedAngleConstraint*)hinge)->getAccumulatedHingeAngle();
...
bool constraintIsEnabled(void* c) { return JOINT(c)->isEnabled(); }

void constraintSetEnabled(void* c, bool en) { JOINT(c)->setEnabled(en); }
...
void* bodyCreate(void* u, void* shape, float mass, float x, float y, float z) {
	...
	if (isDynamic) SHAPE(shape)->calculateLocalInertia(M, localInertia);

	btDefaultMotionState* motionState = new btDefaultMotionState(trans);
	btRigidBody::btRigidBodyConstructionInfo rbInfo(M, motionState,
													SHAPE(shape), localInertia);
...
void bodyApplyImpulse(void* body, Vec* i, Vec* p) {
	BODY(body)->applyImpulse(btVector3(i->x, i->y, i->z),
...
void bodySetRotation(void* body, Vec r) {
	bodySetRotationEuler(body, r.x, r.y, r.z);
}

void bodySetRestitution(void* body, float r) { BODY(body)->setRestitution(r); }
...
void bodySetFriction(void* s, float f) { BODY(s)->setFriction(f); }

float bodyGetFriction(void* s) { return BODY(s)->getFriction(); }
...
void collisionCallback(void* u, void (*callback)(void*, void*, const Vec*,
	...
	for (int i = 0; i < numManifolds; i++) {
		btPersistentManifold* contactManifold =
			UNI(u)->dispatcher->getManifoldByIndexInternal(i);
		...
		for (int j = 0; j < numContacts; j++) {
			...
			if (pt.getDistance() < 0.f) {
				...
				const Vec n = {normalOnB.getX(), normalOnB.getY(),
							   normalOnB.getZ()};
